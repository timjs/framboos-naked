...
In deze opdracht ga je aan de slag om een preemtpive scheduler te maken.
Hierbij moet je gebruik gaan maken van de system timer die de Raspberry Pi biedt.

Dit is een grotere opdracht dan de vorige waarbij de eerste inschatting is dat je er ongeveer 3 keer zoveel aan kwijt bent.
...
4.  Bekijk goed welke informatie van de CPU wordt opgeslagen in het TCB.
    Zoek op internet op welke registers worden opgeslagen.
    Beschrijf voor elk van deze registers (dus SP, PC, LR, R0, en CPSR) de namen en de functie,
    en geef aan waarom deze registers opgeslagen moeten worden in het TCB.
...
In dit geval hebben we de _Iterrupt Request Handler_ of `irq_handler` nodig.
Momenteel is deze geimplemeneerd met een kernel panic.

7.  Bekijk de assembley code in het codeblok voor `irq_handler_asm_wrapper` in `kerel/exception.s` en beantwoord de volgende vragen:
    a.  Welke instructie zorgt er voor dat de inhoud van alle registers op de stack wordt gezet?
        (Hint: het is er echt maar één!)
    b.  Welke instructie springt naar de procedure `void irq_handler(saved_cpu_state_t *)` uit `kernel/handlers.c`?
    c.  Welke instructie zorgt er voor dat de opgeslagen registers weer van de stack teruggeplaatst in de CPU?
...
In dit geval krijgen ze allebij 4MB aan ruimte.
